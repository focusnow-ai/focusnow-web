...
**Inventory update**: 2026-09-26, limited to v0.3.0 activity domains, local retention, client-work records, export and deletion. Verified against `AccountDeletionService`, `AccountExportService` and Desktop local database code. This is a source review, not a new live-cloud or legal compliance audit. The remaining legal analysis and gap list below are the historical 2026-07-02 review unless explicitly updated.
...
| Activity data | app name, window title, website domain, ACTIVE/IDLE/AWAY, durations | Core product function (dashboards, analytics) | Contract performance | Local SQLite (device) + Azure Cosmos DB | Device + EU | Local: no automatic deletion by age. Cloud: 12 months rolling (Cosmos container TTL, `CosmosDbService.RetentionSeconds`), then auto-deleted — **verified active on the live containers** (confirmed 2026-07-03) |
...
| AI daily insight input | that day's activity records (app names, titles, website domains, timestamps) | Generate daily report | Contract performance (feature requested by user) | Azure OpenAI Service (own Azure subscription) | EU | Source logs follow activity-data retention (12 months rolling); the generated report is cached in the `insights` container, see the output row below |
...
| Client-work records | clients and contact details, projects, tasks, tags, timecards, rates, currencies, statements and invoice references | Organize user-entered work and prepare billing records | Review with the operator alongside the public policy | Local SQLite + Azure Cosmos DB | Device + EU | No automatic cloud expiry while the account is active; included in account export and deletion (`AccountExportService`, `AccountDeletionService`) |
...
## 8. v0.3.0 factual update and review sources

The public policy now states the lack of age-based local cleanup, the client-work data categories and their no-expiry cloud policy, and the consent-audit exception to account deletion. Existing G3/G5/G6 notes above are historical: account deletion/export and backend consent audit code now exist. Other gap conclusions were not re-audited in this website update. The operator should review the revised disclosure and retention criteria before publication.

Reference: [GDPR, Article 13](https://eur-lex.europa.eu/eli/reg/2016/679/oj/eng) and [KVKK aydınlatma yükümlülüğü](https://www.kvkk.gov.tr/Icerik/2033/Aydinlatma-Yukumlulugu-).
