...
| `blog_read` | Okuyucu yazının **sonuna** ulaşınca (en az 15 sn sayfada kaldıysa) | `slug`, `locale`, `reading_time_minutes`, `dwell_seconds` | `blog-read-tracker.tsx` |

Not: 15 sn eşiği "gerçek okuma kanıtı" değil, kaza filtresi (End tuşuyla sona
atlayanı eler). Gerçek okuma analizini `dwell_seconds` parametresiyle yaparsın —
ör. "dwell_seconds > 60 olan blog_read'ler". Eşik bilinçli olarak koda değil,
analiz katmanına bırakıldı; olay tanımı hep sabit kalır, karşılaştırılabilirlik
bozulmaz.
...
## 5. Olayları test etme

Yeni bir olay ekledin, çalışıyor mu emin değilsin. **En pratik yol Realtime:**

1. Sitede çerez onayı ver (banner'da kabul).
2. GA4 → Reports → **Realtime** raporunu aç.
3. Sitede olayı tetikle (ör. bir blog yazısını sonuna kadar okuyup 15+ sn
   bekle → `blog_read`).
4. Realtime'daki "Event count by Event name" kartında olay adı 1-2 dakika
   içinde belirmeli. Belirdiyse çalışıyor demektir.

DebugView (Admin → DebugView) daha ayrıntılıdır ama web'de yalnızca
**"Google Analytics Debugger" Chrome eklentisi** açıkken çalışır — URL'e
parametre eklemek yeterli değildir. Hızlı doğrulama için Realtime yeter.

Not: normal raporlara (Engagement → Events) olayların düşmesi 24 saati
bulabilir; anlık doğrulama için Realtime/DebugView kullan. Reklam engelleyici
(uBlock vb.) GA isteklerini keser — test ederken kapat.
