...
Open **Insights**, pick a date, and click **Generate summary**. Two rules worth knowing up front:
...
Generation usually takes a few seconds. If it takes longer, the report is still being prepared in the background, so check back shortly. If a report can't be written, FocusNow tells you why: no activity that day, not enough tracked time, or the summary service is busy. In the last case nothing is lost; try again in a few minutes.
